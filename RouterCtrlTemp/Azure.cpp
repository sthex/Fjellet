...
void AzureClass::Send()
{
	if (hasWifi)
	{
			...
			(int)(millis() - send_interval_ms) >= INTERVAL)
		{
			...
			snprintf(messagePayload, MESSAGE_MAX_LEN, messageData, DEVICE_ID, 321, 
				temperature1, temperature2, temperature3, temperature4, wifiRestarts);
...
int  AzureClass::DeviceMethodCallback(const char *methodName, const unsigned char *payload, int size, unsigned char **response, int *response_size)
{
	...
	else if (strcmp(methodName, "status") == 0)
	{
		...
		sprintf(responseMessage, "{\"Errors\":%d, \"Sent\":%d,\"Mode\":%d,\"t1\":%f,\"t2\":%f,\"RSSI\":\"%d\"}",
			errorCount, messageCount, azureMode,
			temperature1, temperature2, WiFi.RSSI()
		);
